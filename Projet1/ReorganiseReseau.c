...
int main(int argc, char **argv) {
  ...
  if (reorganiseReseau(R))
    printf(
        "La reorganisation du reseau issu de %s fait passer dans chaque arete "
        "moins de %d cables.\n",
        filenamecha, R->gamma);
  else
    printf(
        "La reorganisation du reseau issu de %s viole la contrainte du nombre "
        "maximum de cables par arete\n",
        filenamecha);
